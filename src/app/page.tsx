...
export default function LandingPage() {
  ...
  const handleStudentSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success("挑戦への第一歩、エントリー完了！");
        (e.target as HTMLFormElement).reset();
      } else {
        toast.error("エラーが発生しました: " + (data.error || "送信に失敗しました"));
      }
    } catch {
  ...
  const handleEnterpriseSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setLoading(true);
    const formData = new FormData(e.currentTarget);
    try {
      const res = await fetch("/api/enterprises", {
        ...
        body: JSON.stringify({
          ...
          workEmail: formData.get("email") as string,
          message: formData.get("message") as string,
        }),
      });
      const data = await res.json();
      if (res.ok) {
        toast.success("資料請求を承りました。");
        (e.target as HTMLFormElement).reset();
      } else {
        toast.error("エラーが発生しました: " + (data.error || "送信に失敗しました"));
      }
      ...
      <header className="relative min-h-screen w-full flex flex-col md:flex-row overflow-hidden pt-[60px]">
        ...
        <div className="relative w-full md:w-1/2 min-h-[50vh] md:min-h-full flex items-center justify-center p-xl group" id="students">
          ...
          <div className="relative z-10 text-center md:text-left">
            ...
            <Link href="#registration">
              <button className="student-gradient text-white px-lg py-md rounded-full font-headline-md text-headline-md hover:scale-105 transition-transform shadow-2xl shadow-primary/40 border border-white/20">
                挑戦を始める
              </button>
            </Link>
          </div>
        ...
        <div className="relative w-full md:w-1/2 min-h-[50vh] md:min-h-full flex items-center justify-center p-xl group border-l border-white/5" id="enterprise">
          ...
          <div className="relative z-10 text-center md:text-left">
            ...
            <Link href="#registration">
              <button className="bg-secondary text-on-secondary px-lg py-md rounded-full font-headline-md text-headline-md hover:scale-105 transition-transform shadow-2xl shadow-secondary/40">
                パートナー参画
              </button>
            </Link>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-lg items-center relative z-10">
          ...
          <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-md">
            {[
              ...
              <div key={i} className="glass-card p-md rounded-xl hover:-translate-y-2 transition-all duration-300 hover:border-primary/50 hover:bg-primary/5">
        ...
        <div className="max-w-container-max mx-auto px-gutter relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-xl">
            ...
            <div className="glass-card p-xl rounded-2xl border-primary/40 shadow-2xl shadow-primary/10 relative">
              ...
              <form className="space-y-md" onSubmit={handleStudentSubmit}>
                <div>
            ...
            <div className="glass-card p-xl rounded-2xl border-secondary/40 shadow-2xl shadow-secondary/10 relative">
              ...
              <form className="space-y-md" onSubmit={handleEnterpriseSubmit}>
                <div>
                ...
                <div>
                  ...
                  <input name="contactName" className="w-full bg-white/5 border-0 border-b border-outline-variant focus:border-secondary focus:ring-0 text-on-surface transition-colors p-base" placeholder="山田 太郎" type="text" />
                </div>
                <div>
